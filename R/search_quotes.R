...
#'
#' @return A data frame (also with class \code{'bioacousticsquote'}) object containing all quotes that match the search parameters.
#'
#' @export
#' @seealso \code{\link[base]{agrep}}, \code{\link{bioacousticsquote}}.
#' @examples
...
search_quotes <- function(search,
  ...
  if (length(ix)) {
    data <- data[ix,]
    class(data) <- c("bioacousticsquote", 'data.frame')
    return(data)
...
#'
#' @return A data frame (also with class \code{'bioacousticsquote'}) object
